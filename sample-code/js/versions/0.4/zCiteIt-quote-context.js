            ...
            if (cited_url.length > 3) {
                ...
                function addQuoteToDom(tag_type, json, cited_url) {
                    ...
                    if (tag_type === "q") {
					   ...
					   /* Expand both before and after on click */
					   blockcite.wrapInner("<a href='' " +
                            "onclick='return toggleQuoteBoth(" + json.sha256 + ")' " +
                        " />");
...
function toggleQuoteBoth(sha256) {
	alert("expand both");
	toggleQuote('quote_arrow_up', 'quote_before_' + sha256);
	toggleQuote('quote_arrow_down', 'quote_after_' + sha256);
}
